...
QSettings AppSettings() {
    return QSettings("NewGControl", "NewGControl");
}
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      ...
      m_protocol(std::make_unique<GrblProtocol>()),
      m_programRunner(std::make_unique<ProgramRunner>()) {
    LoadMotionSettings();
...
void MainWindow::BuildUi() {
    ...
    setStyleSheet(
        ...
        "QPushButton#NgcModeAbs:checked { background: #214f38; color: #ecfff2; border: 1px solid #4ca06f; font-weight: 800; }"
        "QPushButton#NgcModeRel:checked { background: #243f63; color: #eef5ff; border: 1px solid #77a7de; font-weight: 800; }"
        "QPushButton#NgcModeAbs:!checked, QPushButton#NgcModeRel:!checked { background: #343a42; color: #8e97a3; border: 1px solid #23272d; font-weight: 600; }"
        "QPushButton#NgcModeAbs:!checked:hover, QPushButton#NgcModeRel:!checked:hover { background: #3d4450; color: #c5ccd6; border-color: #4a5563; }"
        "QPushButton#NgcModeAbs:disabled, QPushButton#NgcModeRel:disabled { background: #2d3238; color: #6a7380; border-color: #252a31; }"
        ...
        "QFrame#NgcMachineControlSep { color: #4a5563; }"
        "QStatusBar { background: #2c3036; color: #9fa6af; border-top: 1px solid #20242a; padding: 6px 14px; }"
        "QStatusBar::item { border: none; }");

    auto *setupMenu = menuBar()->addMenu("&Machine");
    auto *motionParamsAction = setupMenu->addAction("Machine Profile...");
    ...
    auto *grblMenu = menuBar()->addMenu("&GRBL");
    auto *readSettingsAction = grblMenu->addAction("Read $$ Settings");
    auto *readParamsAction = grblMenu->addAction("Read $# Parameters");
    auto *readParserAction = grblMenu->addAction("Read $G Parser State");
    auto *readBuildAction = grblMenu->addAction("Read $I Build Info");
    auto *readStartupAction = grblMenu->addAction("Read $N Startup Blocks");
    grblMenu->addSeparator();
    auto *openSettingsManagerAction = grblMenu->addAction("Settings Manager...");
    grblMenu->addSeparator();
    auto *importSnapshotAction = grblMenu->addAction("Import Snapshot...");
    auto *exportSnapshotAction = grblMenu->addAction("Export Snapshot...");
    connect(readSettingsAction, &QAction::triggered, this, &MainWindow::RequestGrblSettings);
    connect(readParamsAction, &QAction::triggered, this, &MainWindow::RequestGrblParameters);
    connect(readParserAction, &QAction::triggered, this, &MainWindow::RequestGrblParserState);
    connect(readBuildAction, &QAction::triggered, this, &MainWindow::RequestGrblBuildInfo);
    connect(readStartupAction, &QAction::triggered, this, &MainWindow::RequestGrblStartupBlocks);
    connect(openSettingsManagerAction, &QAction::triggered, this, &MainWindow::OpenGrblSettingsManager);
    connect(importSnapshotAction, &QAction::triggered, this, &MainWindow::ImportGrblSnapshot);
    connect(exportSnapshotAction, &QAction::triggered, this, &MainWindow::ExportGrblSnapshot);
    ...
    m_discoverButton = new QPushButton("Add Host", connGroup);
    ...
    ethernetSettingsLayout->addWidget(new QLabel("Host:", m_ethernetSettingsWidget));
    ...
    workH->setObjectName("NgcWorkHSplitter");
    ...
    mainSplitter->setObjectName("NgcMainSplitter");
    ...
    m_absButton->setObjectName("NgcModeAbs");
    m_relButton->setObjectName("NgcModeRel");
    ...
    modeSep->setObjectName("NgcMachineControlSep");
    ...
    statusBar()->showMessage("Ready - GRBL profile probing");
...
void MainWindow::DiscoverEthernetDevices() {
    const QString host = m_deviceCombo ? m_deviceCombo->currentText().trimmed() : QString();
    if (host.isEmpty()) {
        QMessageBox::information(
            this,
            "Add Host",
            "Enter a GRBL controller hostname or IP in the Host field, then click Add Host.");
        return;
    }
    if (m_deviceCombo->findData(host) < 0) {
        m_deviceCombo->addItem(host, host);
    }
    const int idx = m_deviceCombo->findData(host);
    if (idx >= 0) {
        m_deviceCombo->setCurrentIndex(idx);
    }
    AppendLog(QString("Saved host: %1").arg(host));
}
...
void MainWindow::ConnectOrDisconnect() {
    ...
    if (m_usingEthernet) {
        const QString host = m_deviceCombo->currentText().trimmed();
        if (host.isEmpty()) {
            QMessageBox::warning(this, "NewGControl", "Enter an Ethernet host/IP before connecting.");
            return;
        }
        ...
        if (!m_tcpSocket.waitForConnected(3000)) {
            QMessageBox::critical(this, "NewGControl",
    ...
    if (portName.isEmpty()) {
        QMessageBox::warning(this, "NewGControl", "No serial port selected.");
        return;
    }
...
void MainWindow::UpdateConnectionWindowTitle(bool connected) {
    setWindowTitle(connected ? QStringLiteral("NewGControl [Connected]")
                             : QStringLiteral("NewGControl [Disconnected]"));
}
...
void MainWindow::UpdateControllerEnabledStateUi() {
    ...
    m_enableButton->setToolTip(m_controllerEnabled
                                   ...
                                   : QStringLiteral("Unlock/enable motion for this controller profile."));
    m_disableButton->setToolTip(m_controllerEnabled
                                    ? QStringLiteral("Disable motion/spindle outputs for this controller profile.")
...
bool MainWindow::EnsureFeedWithinLimit() {
    ...
    QMessageBox::warning(
        ...
        QString("The requested feed is %1 %2, but the configured motor speed allows a maximum of %3 %2.\n\n"
                "Reduce the feed rate or increase the motor RPM in Machine > Machine Profile.")
...
void MainWindow::DisableControllerBeforeDisconnect() {
    ...
    const QByteArray payload = m_protocol->DisableCommand().toUtf8() + "\n";
    if (m_usingEthernet) {
    ...
    AppendLog(QString("TX: %1 (safe disconnect)").arg(m_protocol->DisableCommand()));
}
...
void MainWindow::SendMotionConfigCommand() {
    // GRBL-family targets do not expose a single motion CONFIG command.
    // Profile values are kept local for feed and axis capability UI.
    SyncActiveMotionSettingsFromPorts();
}
...
void MainWindow::HandleReceivedLine(const QString &line) {
    ...
    m_protocol->ObserveLine(line);
    HandlePossibleGrblLine(line);
    ShowVariantStatus();
...
void MainWindow::HandleTelemetryLine(const QString &line) {
    ...
    if (queueDepth <= 0.0 && active <= 0.0) {
        ...
        if (m_queueDrainStalledPolls >= 10) {
            ...
            ResetProgramTrackingState();
            SetProgramUiRunning(false);
            m_programStatusLabel->setText("Queue stalled; sending emergency reset");
            SendRealtimeBytes(m_protocol->EmergencyRealtimeBytes());
            QMessageBox::warning(
                this,
                "Queue Stalled",
                "The controller reported queued moves but no active motion while the GUI was waiting for the queue to drain.\n\n"
                "An emergency reset was sent. Inspect the mechanism before running again.");
        }
    } else {
        m_queueDrainStalledPolls = 0;
    }
...
void MainWindow::SendStatusPoll() {
    const QStringList commands = m_protocol->StatusPollCommands(IsTelemetryConnected());
...
void MainWindow::SendEnable() {
    SendCommand(m_protocol->EnableCommand());
    QTimer::singleShot(150, this, &MainWindow::SendStatusPoll);
}
void MainWindow::SendDisable() {
    m_controllerEnabled = false;
    SendCommand(m_protocol->DisableCommand());
    UpdateControllerEnabledStateUi();
}
void MainWindow::SendStop() { SendCommand(m_protocol->StopCommand()); }
...
void MainWindow::SendSetAbs() {
    SetMotionModeUi(true);
    SendCommand(m_protocol->SetAbsCommand());
}
void MainWindow::SendSetRel() {
    SetMotionModeUi(false);
    SendCommand(m_protocol->SetRelCommand());
}
void MainWindow::SendZero() { SendCommand(m_protocol->ZeroWorkCommand()); }

void MainWindow::ShowMotionParametersDialog() {
    QDialog dialog(this);
    dialog.setWindowTitle("Machine Profile");
    ...
    auto *singleMotor = new QCheckBox(
        "Single-axis testing profile (no XY coupling assumptions)", &dialog);
    singleMotor->setChecked(m_singleMotorBench);
    singleMotor->setToolTip(
        "Profile flag for bench testing presets only. "
        "It does not send controller-specific configuration commands.");
...
void MainWindow::SendJog(double dx, double dy) {
    ...
    SendActiveUnitsCommand();
    QStringList parts;
    parts.append(QStringLiteral("$J=G91"));
    if (dx != 0.0) {
...
void MainWindow::SendJogZPos() {
    ...
    SendActiveUnitsCommand();
    SendCommand(QString("$J=G91 Z%1 F%2")
                    .arg(step, 0, 'f', 3)
...
void MainWindow::SendJogAPos() {
    const double step = ParseDoubleLine(m_jogStepEdit, 1.0);
    SendActiveUnitsCommand();
    SendCommand(QString("$J=G91 A%1 F%2")
                    .arg(step, 0, 'f', 3)
                    .arg(ParseDoubleLine(m_feedEdit, 500.0), 0, 'f', 3));
}
...
void MainWindow::LoadGCodeProgramImpl(bool focusProgramTabOnSuccess) {
    ...
    m_programPath = path;
    m_programRunner->SetLines(m_programLines);
    m_programIndex = 0;
    m_programRunning = false;
    m_programPaused = false;
    m_programAwaitingAck = false;
...
void MainWindow::PauseGCodeProgram() {
    ...
    m_programRunner->SetPaused(m_programPaused);
    m_pauseProgramButton->setText(m_programPaused ? "Resume" : "Pause");
    SendCommand(m_programPaused ? m_protocol->PauseCommand() : m_protocol->ResumeCommand());
    if (m_programPaused) {
...
void MainWindow::StopGCodeProgram() {
    ...
    m_pollTimer.start();
    SendCommand(m_protocol->StopCommand());
    if (m_usingEthernet) {
        m_tcpSocket.readAll();
    } else if (m_serial.isOpen()) {
        m_serial.readAll();
...
void MainWindow::SendRealtimeBytes(const QByteArray &bytes) {
    if (bytes.isEmpty()) {
        return;
    }
    if (m_usingEthernet) {
...
void MainWindow::HandlePossibleGrblLine(const QString &line) {
    const QString trimmed = line.trimmed();

    int settingId = 0;
    double settingValue = 0.0;
    if (GrblProtocol::ParseGrblSettingLine(trimmed, settingId, settingValue)) {
        m_grblSettings.insert(settingId, settingValue);
        return;
    }

    QString payloadType;
    QString payload;
    if (GrblProtocol::ParseBracketPayload(trimmed, payloadType, payload)) {
        if (payloadType == "G54" || payloadType == "G55" || payloadType == "G56" ||
            payloadType == "G57" || payloadType == "G58" || payloadType == "G59" ||
            payloadType == "G28" || payloadType == "G30" || payloadType == "PRB" ||
            payloadType == "TL0") {
            m_grblParameters.append(trimmed);
        } else if (payloadType == "GC") {
            m_grblParserState = payload;
        } else if (payloadType == "VER" || payloadType == "OPT" || payloadType == "MSG") {
            m_grblBuildInfo = trimmed;
        }
        return;
    }

    const auto status = GrblProtocol::ParseStatusLine(trimmed);
    if (status.valid) {
        if (status.hasWPos) {
            m_vizX = status.wx;
            m_vizY = status.wy;
        ...
        m_controllerEnabled = !status.state.contains("ALARM", Qt::CaseInsensitive);
        UpdateControllerEnabledStateUi();
        return;
    }

    if (trimmed.startsWith("$G", Qt::CaseInsensitive)) {
        m_grblParserState = trimmed;
    } else if (trimmed.startsWith("$N", Qt::CaseInsensitive)) {
        m_grblStartupBlocks.append(trimmed);
    } else if (trimmed.startsWith("grbl", Qt::CaseInsensitive) ||
               trimmed.startsWith("grblHAL", Qt::CaseInsensitive) ||
               trimmed.startsWith("FluidNC", Qt::CaseInsensitive)) {
        m_grblBuildInfo = trimmed;
    }
}

void MainWindow::ShowVariantStatus() {
    if (!statusBar()) {
        return;
    }
    statusBar()->showMessage(QString("Protocol profile: %1").arg(m_protocol->VariantSummary()), 2500);
}

void MainWindow::RequestGrblSettings() {
    if (!IsControllerConnected()) {
        QMessageBox::warning(this, "GRBL Settings", "Connect to a controller first.");
        return;
    }
...
void MainWindow::RequestGrblParameters() {
    if (!IsControllerConnected()) {
        QMessageBox::warning(this, "GRBL Parameters", "Connect to a controller first.");
        return;
    }
    m_grblParameters.clear();
    SendCommand("$#");
}

void MainWindow::RequestGrblParserState() {
    if (!IsControllerConnected()) {
        QMessageBox::warning(this, "GRBL Parser State", "Connect to a controller first.");
        return;
    }
    SendCommand("$G");
}

void MainWindow::RequestGrblBuildInfo() {
    if (!IsControllerConnected()) {
        QMessageBox::warning(this, "GRBL Build Info", "Connect to a controller first.");
        return;
    }
    SendCommand("$I");
}

void MainWindow::RequestGrblStartupBlocks() {
    if (!IsControllerConnected()) {
        QMessageBox::warning(this, "GRBL Startup Blocks", "Connect to a controller first.");
        return;
    }
    m_grblStartupBlocks.clear();
    SendCommand("$N");
}

void MainWindow::OpenGrblSettingsManager() {
    QDialog dialog(this);
    dialog.setWindowTitle("GRBL Settings Manager");
    dialog.resize(760, 520);

    auto *layout = new QVBoxLayout(&dialog);
    auto *summaryLabel = new QLabel(
        QString("Controller profile: %1").arg(m_protocol->VariantSummary()), &dialog);
    ...
    table->setRowCount(m_grblSettings.size());
    int row = 0;
    for (auto it = m_grblSettings.cbegin(); it != m_grblSettings.cend(); ++it, ++row) {
        auto *idItem = new QTableWidgetItem(QString("$%1").arg(it.key()));
    ...
    auto *miscLabel = new QLabel(&dialog);
    QStringList meta;
    if (!m_grblParserState.isEmpty()) {
        meta << QString("Parser: %1").arg(m_grblParserState);
    }
    if (!m_grblBuildInfo.isEmpty()) {
        meta << QString("Build: %1").arg(m_grblBuildInfo);
    }
    if (!m_grblParameters.isEmpty()) {
        meta << QString("Parameters loaded: %1").arg(m_grblParameters.size());
    }
    if (!m_grblStartupBlocks.isEmpty()) {
        meta << QString("Startup blocks loaded: %1").arg(m_grblStartupBlocks.size());
    }
    miscLabel->setText(meta.isEmpty() ? "No metadata captured yet. Use the GRBL menu to query controller state." : meta.join(" | "));
    layout->addWidget(miscLabel);

    auto *buttons = new QDialogButtonBox(
        QDialogButtonBox::Close | QDialogButtonBox::Apply | QDialogButtonBox::Reset, &dialog);
    auto *readNow = buttons->addButton("Read All", QDialogButtonBox::ActionRole);
    QObject::connect(readNow, &QPushButton::clicked, this, [this]() {
        RequestGrblSettings();
        RequestGrblParameters();
        RequestGrblParserState();
        RequestGrblBuildInfo();
        RequestGrblStartupBlocks();
    });
    QObject::connect(buttons->button(QDialogButtonBox::Reset), &QPushButton::clicked, this, [this, table]() {
        Q_UNUSED(table);
        RequestGrblSettings();
    });
    QObject::connect(buttons->button(QDialogButtonBox::Apply), &QPushButton::clicked, this, [this, table]() {
        if (!IsControllerConnected()) {
            QMessageBox::warning(this, "Apply GRBL Settings", "Connect to a controller first.");
            return;
        }
        for (int row = 0; row < table->rowCount(); ++row) {
            ...
            bool idOk = false;
            const int id = idItem->text().mid(1).toInt(&idOk);
            bool valueOk = false;
            const double value = valueItem->text().toDouble(&valueOk);
            if (!idOk || !valueOk) {
                continue;
            }
            SendCommand(QString("$%1=%2").arg(id).arg(value, 0, 'f', 3));
            m_grblSettings.insert(id, value);
        }
        if (m_protocol->CurrentVariant() == GrblProtocol::Variant::Grbl11 ||
            m_protocol->CurrentVariant() == GrblProtocol::Variant::GrblHal ||
            m_protocol->CurrentVariant() == GrblProtocol::Variant::FluidNc) {
            SendCommand("$");
        }
    });
...
void MainWindow::ExportGrblSnapshot() {
    const QString filePath = QFileDialog::getSaveFileName(
        this,
        "Export GRBL Snapshot",
        AppSettings().value("grbl/lastSnapshotPath").toString(),
        "JSON files (*.json);;All files (*.*)");
    if (filePath.isEmpty()) {
        return;
    }

    GrblSnapshot snapshot;
    snapshot.variant = m_protocol->VariantName();
    snapshot.settings = m_grblSettings;
    snapshot.parameters = m_grblParameters;
    snapshot.parserState = m_grblParserState;
    snapshot.startupBlocks = m_grblStartupBlocks;
    snapshot.buildInfo = m_grblBuildInfo;

    QString error;
    if (!GrblSnapshotStore::SaveToFile(filePath, snapshot, error)) {
        QMessageBox::critical(this, "Export GRBL Snapshot",
                              QString("Could not save snapshot:\n%1").arg(error));
        return;
    }
    AppSettings().setValue("grbl/lastSnapshotPath", filePath);
    AppendLog(QString("Exported GRBL snapshot: %1").arg(filePath));
}

void MainWindow::ImportGrblSnapshot() {
    const QString filePath = QFileDialog::getOpenFileName(
        this,
        "Import GRBL Snapshot",
        AppSettings().value("grbl/lastSnapshotPath").toString(),
        "JSON files (*.json);;All files (*.*)");
    if (filePath.isEmpty()) {
        return;
    ...
    GrblSnapshot snapshot;
    QString error;
    if (!GrblSnapshotStore::LoadFromFile(filePath, snapshot, error)) {
        QMessageBox::critical(this, "Import GRBL Snapshot",
                              QString("Could not read snapshot:\n%1").arg(error));
        return;
    }
    m_grblSettings = snapshot.settings;
    m_grblParameters = snapshot.parameters;
    m_grblParserState = snapshot.parserState;
    m_grblStartupBlocks = snapshot.startupBlocks;
    m_grblBuildInfo = snapshot.buildInfo;
    AppSettings().setValue("grbl/lastSnapshotPath", filePath);
    AppendLog(QString("Imported GRBL snapshot: %1 (%2 settings)")
                  .arg(filePath)
                  .arg(m_grblSettings.size()));
}
