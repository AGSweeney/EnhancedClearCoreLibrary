...
QSettings AppSettings() {
    return QSettings("ClearHPGL", "ClearHPGL");
}
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      ...
      m_lockTopDownViewCheck(nullptr),
      m_viewportLeftDragOrbitCheck(nullptr),
      m_vizX(0.0),
      m_vizY(0.0),
      m_vizZ(0.0),
      m_vizA(0.0),
...
void MainWindow::BuildUi() {
    ...
    auto *openDxfAction = programMenu->addAction("Open DXF...");
    connect(openDxfAction, &QAction::triggered, this, &MainWindow::OpenDxfFromMenu);
    m_saveDxfGeometryAction = programMenu->addAction(QStringLiteral("Save DXF geometry…"));
    m_saveDxfGeometryAction->setEnabled(false);
    connect(m_saveDxfGeometryAction, &QAction::triggered, this, &MainWindow::SaveDxfGeometryAs);
    ...
    auto *programGroup = new QGroupBox("Program (G-code or DXF → HPGL)", programTab);
    ...
    m_loadProgramButton = new QPushButton("Load G-code", programGroup);
    m_loadDxfButton = new QPushButton("Open DXF", programGroup);
    ...
    programControls->addWidget(m_loadDxfButton);
    ...
    m_programPreview->setPlaceholderText(
        "Load G-code (.nc, .tap, .gcode) or an ASCII DXF (.dxf). DXF is converted to HPGL for preview; path shows in 3D.");
    ...
    pos3dHeader->addWidget(new QLabel(
        "Z up · L-drag rotate (optional) · Top-down locks tilt · Shift/middle/right = pan · wheel = zoom", pos3dGroup));
    pos3dHeader->addStretch();
    m_viewportPathTraceCheck = new QCheckBox("Path", pos3dGroup);
    m_viewportPathTraceCheck->setToolTip(
        "Draw the loaded file in 3D (yellow = cut / pen-down, gray = rapid / pen-up). "
        "G-code is parsed for motion; DXF is tessellated then shown in drawing units. "
        "The tool arrow follows live DRO / telemetry when connected. "
        "Red crosshairs flag laser-relevant geometry: open contours (DXF), overlapping or crossing cuts, "
        "and tight corners vs Corner DVmax (when DVmax > 0). Hover for details; right-click a crosshair for "
        "copy actions and DXF quick fixes (close contour / merge across pen-up) when available.");
    ...
    m_lockTopDownViewCheck = new QCheckBox("Top-down", pos3dGroup);
    m_lockTopDownViewCheck->setToolTip(
        "Plan view: camera looks straight down +Z onto XY, with world +X to the right and +Y up on screen "
        "(azimuth snaps to 0 when you turn this on or hit Reset view while top-down). "
        "Elevation stays locked; in-plane spin uses left-drag only if “L-drag rotate” is checked. Pan and zoom unchanged.");
    pos3dHeader->addWidget(m_lockTopDownViewCheck);
    m_viewportLeftDragOrbitCheck = new QCheckBox("L-drag rotate", pos3dGroup);
    m_viewportLeftDragOrbitCheck->setToolTip(
        "When checked, dragging with the left mouse button orbits the camera (azimuth; elevation too unless Top-down). "
        "When unchecked (default), left-drag does nothing—use Shift+left (or middle/right) to pan so the view does not spin accidentally.");
    m_viewportLeftDragOrbitCheck->setChecked(false);
    pos3dHeader->addWidget(m_viewportLeftDragOrbitCheck);
    ...
    connect(m_lockTopDownViewCheck, &QCheckBox::toggled, this, [this](bool on) {
        if (m_positionView3D) {
            m_positionView3D->setTopDownLocked(on);
        }
        AppSettings().setValue("ui/viewportTopDownLocked", on);
    });
    connect(m_viewportLeftDragOrbitCheck, &QCheckBox::toggled, this, [this](bool on) {
        if (m_positionView3D) {
            m_positionView3D->setLeftDragOrbitEnabled(on);
        }
        AppSettings().setValue("ui/viewportLeftDragOrbitEnabled", on);
    });
    m_positionView3D->setPathTraceEnabled(m_viewportPathTraceCheck->isChecked());
    connect(m_positionView3D, &PositionView3D::pathIssueQuickFixCloseContour, this,
            &MainWindow::OnPathIssueQuickFixCloseContour);
    connect(m_positionView3D, &PositionView3D::pathIssueQuickFixMergeAcrossRapid, this,
            &MainWindow::OnPathIssueQuickFixMergeAcrossRapid);
    ...
    connect(m_loadDxfButton, &QPushButton::clicked, this, &MainWindow::LoadDxfFile);
...
void MainWindow::ConnectOrDisconnect() {
    ...
    if (m_usingEthernet) {
        if (m_deviceCombo->currentData().isNull()) {
            QMessageBox::warning(this, "ClearHPGL", "No Ethernet device selected. Click Discover first.");
            return;
        }
        ...
        if (!m_tcpSocket.waitForConnected(3000)) {
            QMessageBox::critical(this, "ClearHPGL",
    ...
    if (portName.isEmpty()) {
        QMessageBox::warning(this, "ClearHPGL", "No serial port selected.");
        return;
    }
...
void MainWindow::SetConnectedUi(bool connected) {
    ...
    if (m_loadDxfButton) {
        m_loadDxfButton->setEnabled(true);
    }
...
void MainWindow::UpdateConnectionWindowTitle(bool connected) {
    setWindowTitle(connected ? QStringLiteral("ClearHPGL [Connected]")
                             : QStringLiteral("ClearHPGL [Disconnected]"));
}
...
void MainWindow::ClearDxfPreviewState() {
    m_dxfPreviewActive = false;
    m_dxfPolylinesMm.clear();
    m_dxfRapidSegmentsMm.clear();
    m_dxfHpglText.clear();
    m_dxfSourcePath.clear();
    m_dxfInsunits = 0;
    m_dxfDrawingToMmScale = 25.4;
}

void MainWindow::LoadDxfFile() {
    LoadDxfFileImpl(false);
}

void MainWindow::OpenDxfFromMenu() {
    LoadDxfFileImpl(true);
}

void MainWindow::LoadDxfFileImpl(bool focusProgramTabOnSuccess) {
    const QString path = QFileDialog::getOpenFileName(
        this,
        "Open DXF",
        AppSettings().value("program/lastDirectory").toString(),
    ...
    DxfImportOutput imported;
    if (!ImportDxfFile(path, &imported)) {
        QMessageBox::warning(this, "Open DXF",
                             imported.errorMessage.isEmpty() ? QStringLiteral("Import failed.")
    ...
    m_dxfPreviewActive = true;
    m_dxfSourcePath = path;
    m_dxfInsunits = imported.insunits;
    m_dxfDrawingToMmScale = imported.drawingToMmScale;
    m_dxfPolylinesMm = std::move(imported.polylinesMm);
    m_dxfRapidSegmentsMm = std::move(imported.rapidSegmentsMm);
    m_dxfHpglText = std::move(imported.hpgl);

    AppSettings().setValue("program/lastDirectory", QFileInfo(path).absolutePath());

    m_programPreview->setPlainText(m_dxfHpglText);
    m_programStatusLabel->setText(QStringLiteral("DXF → HPGL (%1 polylines)")
                                      .arg(m_dxfPolylinesMm.size()));
    AppendLog(QStringLiteral("Loaded DXF: %1 — HPGL preview %2 characters; use \"Save DXF geometry\" to "
                              "export edits after quick fixes.")
                  .arg(path)
                  .arg(m_dxfHpglText.size()));
...
void MainWindow::StartGCodeProgram() {
    if (m_dxfPreviewActive) {
        QMessageBox::information(
            this,
            "Run Program",
            "The preview is a DXF → HPGL conversion for plotting only. "
            "Use \"Load G-code\" to load machine commands for Run.");
        return;
    }
    if (!IsControllerConnected()) {
...
void MainWindow::SetProgramUiRunning(bool running) {
    ...
    if (m_loadDxfButton) {
        m_loadDxfButton->setEnabled(!running);
    }
...
void MainWindow::RebuildProgramVizKinematics() {
    ...
    const bool hasDxfPath = m_dxfPreviewActive && !m_dxfPolylinesMm.isEmpty();
    const bool hasGcodePath = !m_programLines.isEmpty();
    if (m_saveDxfGeometryAction) {
        m_saveDxfGeometryAction->setEnabled(hasDxfPath);
    }
    if (m_viewportPathTraceCheck) {
        m_viewportPathTraceCheck->setEnabled(hasDxfPath || hasGcodePath);
    }
    ...
    auto activeSpmmXY = [&](double *spmmX, double *spmmY) {
        *spmmX = m_stepsPerUnit;
        *spmmY = m_stepsPerUnit;
        for (const auto &port : m_motorPorts) {
            if (!port.enabled) {
                continue;
            }
    ...
    auto applyPathIssueMarkers = [&](const QVector<QVector<QVector3D>> &cutGui, bool dxfSource,
                                     const QVector<QVector3D> *rapidsGui) {
        double spmmX = m_stepsPerUnit;
        double spmmY = m_stepsPerUnit;
        activeSpmmXY(&spmmX, &spmmY);
        const double feedMmMin =
            (m_stepsPerUnit > 0.0) ? ((m_velocity * 60.0) / m_stepsPerUnit) : 0.0;
        QVector<PathIssueMarker> markers;
        BuildPathIssueMarkers(cutGui, dxfSource, m_guiUnitsInches, m_junctionDVmax, spmmX, spmmY, feedMmMin,
                              rapidsGui, &markers);
        m_positionView3D->setPathIssues(markers);
    };

    if (hasDxfPath) {
        constexpr float kMmPerInViz = 25.4f;
        auto mmToGui = [this](const QVector3D &pMm) -> QVector3D {
            if (m_guiUnitsInches) {
                return QVector3D(float(double(pMm.x()) / double(kMmPerInViz)),
                                 float(double(pMm.y()) / double(kMmPerInViz)),
                                 float(double(pMm.z()) / double(kMmPerInViz)));
            }
            return pMm;
        };
        QVector<QVector<QVector3D>> cutGui;
        cutGui.reserve(m_dxfPolylinesMm.size());
        for (const QVector<QVector3D> &stripMm : m_dxfPolylinesMm) {
            QVector<QVector3D> strip;
            strip.reserve(stripMm.size());
            for (const QVector3D &p : stripMm) {
                strip.append(mmToGui(p));
            }
            cutGui.append(strip);
        }
        QVector<QVector3D> rapidGui;
        rapidGui.reserve(m_dxfRapidSegmentsMm.size());
        for (const QVector3D &p : m_dxfRapidSegmentsMm) {
            rapidGui.append(mmToGui(p));
        }
        m_positionView3D->setProgramPathSplit(cutGui, rapidGui);
        applyPathIssueMarkers(cutGui, true, &rapidGui);
        UpdateToolVizForViewport();
        return;
    }
    ...
    applyPathIssueMarkers(r.cutStrips, false, &r.rapidSegments);
    UpdateToolVizForViewport();
}

void MainWindow::OnPathIssueQuickFixCloseContour(int stripIndex) {
    if (!m_dxfPreviewActive) {
        return;
    }
    if (stripIndex < 0 || stripIndex >= m_dxfPolylinesMm.size()) {
        return;
    }
    QVector<QVector3D> &strip = m_dxfPolylinesMm[stripIndex];
    if (strip.size() < 3) {
        return;
    }
    constexpr float tolMm = 0.09f;
    if ((strip.first() - strip.last()).length() <= tolMm) {
        AppendLog(QStringLiteral("Quick fix: contour on strip %1 already closed (skipped).").arg(stripIndex));
        return;
    }
    const QVector3D endPrev = strip.at(strip.size() - 2);
    const QVector3D endP = strip.last();
    const QVector3D startP = strip.first();
    const QVector3D startNext = strip.at(1);
    const QVector<QVector3D> mid = gapBezierBridgeInterior(endPrev, endP, startP, startNext, 26);
    if (!mid.isEmpty()) {
        for (const QVector3D &p : mid) {
            strip.append(p);
        }
    } else {
        const int n = 12;
        for (int k = 1; k < n; ++k) {
            ...
            strip.append(endP * (1.f - t) + startP * t);
        }
    }
    strip.append(startP);
    m_dxfHpglText = BuildHpglFromPolylinesMm(m_dxfPolylinesMm);
    if (m_programPreview) {
        m_programPreview->setPlainText(m_dxfHpglText);
    }
    AppendLog(QStringLiteral("DXF quick fix: closed strip %1 with a tangent-smoother bridge to the start "
                              "(preview updated; use Program → Save DXF geometry to write a DXF).")
                  .arg(stripIndex));
    RebuildProgramVizKinematics();
    PromptSaveDxfAfterGeometryFix();
}

void MainWindow::OnPathIssueQuickFixMergeAcrossRapid(int rapidPairIndex) {
    if (!m_dxfPreviewActive) {
        return;
    }
    if (rapidPairIndex < 0 || rapidPairIndex + 1 >= m_dxfPolylinesMm.size()) {
        return;
    }
    const int flatIdx = rapidPairIndex * 2;
    if (flatIdx + 1 >= m_dxfRapidSegmentsMm.size()) {
        return;
    }
    QVector<QVector3D> a = m_dxfPolylinesMm[rapidPairIndex];
    const QVector<QVector3D> &b = m_dxfPolylinesMm[rapidPairIndex + 1];
    constexpr float tol = 0.06f; // mm
    const bool meet = (a.last() - b.first()).lengthSquared() <= tol * tol;
    if (!meet) {
        const QVector3D endPrev = a.size() >= 2 ? a.at(a.size() - 2) : a.last();
        const QVector3D endP = a.last();
        const QVector3D startP = b.first();
        const QVector3D startNext = b.size() >= 2 ? b.at(1) : (startP + QVector3D(0.02f, 0.f, 0.f));
        const QVector<QVector3D> mid = gapBezierBridgeInterior(endPrev, endP, startP, startNext, 26);
        if (!mid.isEmpty()) {
            for (const QVector3D &p : mid) {
                a.append(p);
            ...
            for (int k = 1; k < n; ++k) {
                const float t = float(k) / float(n);
                a.append(endP * (1.f - t) + startP * t);
            }
        }
    }
    const int startBi = meet ? 1 : 0;
    for (int i = startBi; i < b.size(); ++i) {
        a.append(b.at(i));
    }
    m_dxfPolylinesMm[rapidPairIndex] = a;
    m_dxfPolylinesMm.removeAt(rapidPairIndex + 1);
    m_dxfRapidSegmentsMm.remove(flatIdx, 2);
    m_dxfHpglText = BuildHpglFromPolylinesMm(m_dxfPolylinesMm);
    if (m_programPreview) {
        m_programPreview->setPlainText(m_dxfHpglText);
    }
    AppendLog(QStringLiteral("DXF quick fix: merged strips %1 and %2 across pen-up (tangent bridge where "
                              "needed; use Program → Save DXF geometry to export).")
                  .arg(rapidPairIndex)
                  .arg(rapidPairIndex + 1));
    RebuildProgramVizKinematics();
    PromptSaveDxfAfterGeometryFix();
}

QString MainWindow::SuggestedDxfSavePath() const {
    if (!m_dxfSourcePath.isEmpty()) {
        const QFileInfo fi(m_dxfSourcePath);
        return fi.absolutePath() + QLatin1Char('/') + fi.completeBaseName() + QStringLiteral("_fixed.dxf");
    }
    const QString dir = AppSettings().value("program/lastDirectory").toString();
    return dir.isEmpty() ? QStringLiteral("geometry_fixed.dxf")
                         : (QDir(dir).filePath(QStringLiteral("geometry_fixed.dxf")));
}

void MainWindow::PromptSaveDxfAfterGeometryFix() {
    const auto r = QMessageBox::question(
        this,
        QStringLiteral("Save DXF"),
        QStringLiteral("Save the corrected geometry to a new DXF file? Your original file is not overwritten "
                       "until you confirm a path."),
...
void MainWindow::SaveDxfGeometryAs() {
    if (!m_dxfPreviewActive || m_dxfPolylinesMm.isEmpty()) {
        QMessageBox::information(this, QStringLiteral("Save DXF"),
                                 QStringLiteral("Load a DXF first."));
        return;
    }
    const QString suggested = SuggestedDxfSavePath();
    const QString outPath = QFileDialog::getSaveFileName(
        this,
        QStringLiteral("Save DXF geometry"),
        suggested,
    ...
    QString err;
    if (!ExportAsciiDxfMinimal(outPath, m_dxfPolylinesMm, m_dxfInsunits, m_dxfDrawingToMmScale, &err)) {
        QMessageBox::warning(this, QStringLiteral("Save DXF"),
                             err.isEmpty() ? QStringLiteral("Export failed.") : err);
        return;
    }
    m_dxfSourcePath = outPath;
    AppendLog(QStringLiteral("Wrote DXF geometry: %1").arg(outPath));
}
...
void MainWindow::LoadWindowUiSettings() {
    ...
    if (m_lockTopDownViewCheck && m_positionView3D) {
        const bool td = AppSettings().value("ui/viewportTopDownLocked", false).toBool();
        const QSignalBlocker blocker(m_lockTopDownViewCheck);
        m_lockTopDownViewCheck->setChecked(td);
        m_positionView3D->setTopDownLocked(td);
    }
    if (m_viewportLeftDragOrbitCheck && m_positionView3D) {
        const bool orbit = AppSettings().value("ui/viewportLeftDragOrbitEnabled", false).toBool();
        const QSignalBlocker blocker(m_viewportLeftDragOrbitCheck);
        m_viewportLeftDragOrbitCheck->setChecked(orbit);
        m_positionView3D->setLeftDragOrbitEnabled(orbit);
    }
}
...
void MainWindow::SaveWindowUiSettings() {
    ...
    if (m_lockTopDownViewCheck) {
        AppSettings().setValue("ui/viewportTopDownLocked", m_lockTopDownViewCheck->isChecked());
    }
    if (m_viewportLeftDragOrbitCheck) {
        AppSettings().setValue("ui/viewportLeftDragOrbitEnabled", m_viewportLeftDragOrbitCheck->isChecked());
    }
}
