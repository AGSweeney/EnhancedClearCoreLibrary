/*
 * Title: MoveQuadrature
 *
 * Objective:
 *    This example demonstrates control of a ClearPath motor in Quadrature
 *    Input mode using ClearCore's CPM_MODE_QUAD_AB motion output.
 *
 * Description:
 ...
 *    for Step and Direction / Quadrature (In MSP select Mode>>Step and
 *    Direction, then set Input Format to "Quadrature").
 ...
 *    button).
 * 4. Set the Input Format in MSP to "Quadrature". Prefer leaving "Has Detents"
 *    unchecked so each quadrature edge is one count (matching one ClearCore
 *    step).
 *
 ...
 *
 * ** Note: Use MotorManager::CLOCK_RATE_NORMAL (shown below). CLOCK_RATE_HIGH
 *    is not recommended for ClearPath or for QUAD_AB burst timing.
 *
 * Links:
...
int main() {
    // Sets the input clocking rate. NORMAL is required for reliable QUAD_AB
    // edge timing within each sample period (and is recommended for ClearPath).
    MotorMgr.MotorInputClocking(MotorManager::CLOCK_RATE_NORMAL);

    // Sets all motor connectors into quadrature A/B motion-output mode.
    ...
    SerialPort.SendLine("Motor Enabled (QUAD_AB)");
 ...
 *
 *    Command quadrature counts to move the motor's current position to the
 *    absolute position specified by "position"
 *    Prints the move status to the USB serial port
 *    Returns when HLFB asserts (indicating the motor has reached the commanded
 *    position)
 *
 * Parameters:
 *    int position  - The absolute position, in quadrature counts, to move to
 *
